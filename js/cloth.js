...
Constraint.prototype.enforce = function() {
  ...
  let vab = this.p1.position.clone().sub(this.p2.position);
  let vabMag = vab.length();

  // compute correction
  let vCorr = vab.multiplyScalar((vabMag - this.distance) / vabMag);

  // apply correction
  this.p2.position.add(vCorr.clone().multiplyScalar(0.5));
  this.p1.position.sub(vCorr.clone().multiplyScalar(0.5));
...
function Cloth(w, h, l) {
  ...
  if (SceneParams.structuralSprings) {
    ...
    for (let j = 0; j <= h; j++) {
      for (let i = 0; i <= w; i++) {
        // particle and particle to the right
        if (j < h && i != 0 && i != w) {
          constraints.push(
            new Constraint(particles[index(i, j)], particles[index(i, j+1)], this.restDistance)
          );
        }

        // particle and particle below
        if (i < w && j != 0 && j != h) {
          constraints.push(
            new Constraint(particles[index(i, j)], particles[index(i+1, j)], this.restDistance)
          );
        }
      }
    }
  ...
  if (SceneParams.shearSprings) {
    ...
    for (let j = 0; j < h; j++) {
      for (let i = 0; i < w; i++) {
        // particle and particle to the lower right
        constraints.push(
          new Constraint(particles[index(i, j)], particles[index(i+1, j+1)], this.restDistance * this.restDistanceS)
        );

        // particle and particle to the lower left
        constraints.push(
          new Constraint(particles[index(i+1, j)], particles[index(i, j+1)], this.restDistance * this.restDistanceS)
        );
      }
    }
  ...
  if (SceneParams.bendingSprings) {
    ...
    for (let j = 0; j <=h; j++) {
      for (let i = 0; i <= w; i++) {
        // particle and particle 2 to the right
        if (j < h-1) {
          constraints.push(
            new Constraint(particles[index(i, j)], particles[index(i, j+2)], this.restDistance * this.restDistanceB)
          );
        }

        if (i < w-1) {
          constraints.push(
            new Constraint(particles[index(i, j)], particles[index(i+2, j)], this.restDistance * this.restDistanceB)
          );
        }
      }
    }
...
function handleImpactEvents(event) {
  ...
  if (event.keyCode === 37) { // left
    p.position.add(keyMap.ArrowLeft.clone().multiplyScalar(scale));
  } else if (event.keyCode === 38) { // up
    p.position.add(keyMap.ArrowUp.clone().multiplyScalar(scale));
  } else if (event.keyCode === 39) { // right
    p.position.add(keyMap.ArrowRight.clone().multiplyScalar(scale));
  } else if (event.keyCode === 40) { // down
    p.position.add(keyMap.ArrowDown.clone().multiplyScalar(scale));
  };
...
Cloth.prototype.applyGravity = function() {
  ...
  for (let i = 0; i < particles.length; i++) {
    let force = GRAVITY * -1 * particles[i].mass;
    let forceV = new THREE.Vector3(0.0, force, 0.0);
    particles[i].addForce(forceV);
  }
...
Cloth.prototype.applyWind = function(windStrength) {
  ...
  // let wt = windStrength + (Math.cos(time));
  // windForce = new THREE.Vector3(1,1,1).normalize().multiplyScalar(wt);
...
Cloth.prototype.applyCustom = function(strength, rate) {
  ...
  for (let i = 0; i < particles.length; i++) {
    let force = GRAVITY * particles[i].mass * strength;
    let forceV = new THREE.Vector3(0, force, force);
    particles[i].addForce(forceV);
  }
